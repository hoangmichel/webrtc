...

{
  'conditions': [
    ['OS=="win" and chromium_win_pch==1', {
        'target_defaults': {
          'msvs_precompiled_header': '<(webrtc_depot_dir)/build/precompile.h',
          'msvs_precompiled_source': '<(webrtc_depot_dir)/build/precompile.cc',
          'sources': ['<(webrtc_depot_dir)/build/precompile.cc'],
        }
      }],
  ],
}
